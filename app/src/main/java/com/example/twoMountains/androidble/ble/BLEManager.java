package com.example.twoMountains.androidble.ble;
...
import com.example.twoMountains.androidble.util.ClsUtils;
...
public class BLEManager  {
    ...
    private List<String> foundDevices = new ArrayList<>();//已发现的设备
    ...
    private BluetoothAdapter.LeScanCallback leScanCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(BluetoothDevice bluetoothDevice, int rssi, byte[] bytes) {
            ...
            if(bluetoothDevice == null)
                return;
            // 检查设备是否已存在于列表中，若存在则忽略
            if (foundDevices.contains(bluetoothDevice.getAddress()))
                return;
            // 将新设备添加到列表中
    ...
    private BluetoothGattCallback bluetoothGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if(newState == BluetoothGatt.STATE_CONNECTED){
                ...
                if(status == 133){
                    ...
                    if(onBleConnectListener != null){
                        gatt.close();
                        onBleConnectListener.onConnectFailure(gatt,bluetoothDevice,"Connection exception！",status);  //133连接异常 异常断开
                    ...
                    if(onBleConnectListener != null){
                        gatt.close();
                        onBleConnectListener.onConnectFailure(gatt,bluetoothDevice,"Connection successful, service not found disconnected！",status); //62没有发现服务 异常断开
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if(setupService(gatt,serviceUUID,readUUID,writeUUID)){
                ...
                if(onBleConnectListener != null){
                    onBleConnectListener.onServiceDiscoveryFailed(gatt,gatt.getDevice(),"Abnormal service feature acquisition");  //发现服务失败回调
                }
            }
        }
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            ...
            if(status == BluetoothGatt.GATT_SUCCESS){  //设置MTU成功
                ...
                if(onBleConnectListener != null){
                    onBleConnectListener.onMTUSetSuccess("New MTU value after setting = " + (mtu-3) + "   status = " + status,mtu - 3);  //MTU设置成功
                }

            }else if(status == BluetoothGatt.GATT_FAILURE){  //设置MTU失败
                Log.e(TAG,"设置MTU值失败：" + (mtu-3) + ",status" + status);
                if(onBleConnectListener != null){
                    onBleConnectListener.onMTUSetFailure("Setting MTU value failed：" + (mtu-3) + "   status：" + status);  //MTU设置失败
                }
            }
    ...
    private Runnable connectOutTimeRunnable = new Runnable() {
        ...
        public void run() {
            ...
            //连接超时当作连接失败回调
            if(onBleConnectListener != null){
                onBleConnectListener.onConnectFailure(mBluetoothGatt,curConnDevice,"connection timed out！",-1);  //连接失败回调
            }
        }
    };
    ...
    private Runnable serviceDiscoverOutTimeRunnable = new Runnable() {
        ...
        public void run() {
            ...
            //发现服务超时当作连接失败回调
            if(onBleConnectListener != null){
                onBleConnectListener.onConnectFailure(mBluetoothGatt,curConnDevice,"Discovery service timeout！",-1);  //连接失败回调
            }
        }
    };
    ...
    private boolean setupService(BluetoothGatt bluetoothGatt,String serviceUUID,String readUUID,String writeUUID) {
        ...
        // Log.d(TAG, "service = " + service.getUuid().toString());
            String serviceuuid =serviceUUID.toLowerCase(Locale.ROOT);
            Log.d(TAG, "1        "+serviceuuid);
            Log.d(TAG, "2        "+service.getUuid().toString());
            Log.d(TAG, "3        "+serviceUUID);
            if (service.getUuid().toString().equals(serviceUUID)||service.getUuid().toString().equals(serviceuuid)) {
...
//        bluetoothGattService = bleManager.getBluetoothGattService(bluetoothGatt,ConsData.MY_BLUETOOTH4_UUID);
        ...
        for (BluetoothGattCharacteristic characteristic : bluetoothGattService.getCharacteristics()) {
            if (characteristic.getUuid().toString().equals(readUUID)||characteristic.getUuid().toString().equals(readUUID.toLowerCase(Locale.ROOT))) {  //读特征
                readCharacteristic = characteristic;
            } else if (characteristic.getUuid().toString().equals(writeUUID)||characteristic.getUuid().toString().equals(writeUUID.toLowerCase(Locale.ROOT))) {  //写特征
    ...
    public boolean sendMessage(String msg){
        ...
        try {
            ...
            System.out.println("Invalid hex string provided: " + e.getMessage());
        }
